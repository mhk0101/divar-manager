...
class LoginState(str, Enum):
    """وضعیت‌های ممکن در طول فرآیند Login شیپور."""
...
class LoginResult:
    """خروجی نهایی فرآیند Login شیپور."""
...
def normalize_otp(raw: str) -> str:
    """
    تبدیل ورودی کد تأیید به رشته‌ی ۴ رقمی انگلیسی.
    """
    fa_to_en = str.maketrans("۰۱۲۳۴۵۶۷۸۹٠١٢٣٤٥٦٧٨٩", "01234567890123456789")
    cleaned = raw.translate(fa_to_en).strip()
    cleaned = re.sub(r"\s+", "", cleaned)
    ...
    if not re.fullmatch(r"\d{4}", cleaned):
        raise ValueError(f"Verification code must be exactly 4 digits, got: {raw!r}")
    return cleaned
